...
async def export_pdf(body: PreviewRequest):
    ...
    if not os.path.exists(edge_path):
        edge_path = shutil.which("msedge") or shutil.which("chrome") or shutil.which("google-chrome")
    ...
    # Serverless fallback for cloud environments (Vercel): Inject auto-print script
    print_html = html.replace("</body>", "<script>window.onload = function() { window.print(); };</script></body>")
    return HTMLResponse(content=print_html, headers={"X-PDF-Fallback": "true"})
